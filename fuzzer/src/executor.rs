...
pub trait ConfigTrack {
    fn setpipe(&mut self, track_read: RawFd, track_write: RawFd) -> &mut Self;
}
...
impl Executor {
    ...
    pub fn track(&mut self, id: usize, buf: &Vec<u8>) -> (std::process::Child, RawFd) {
        //FIXME
        let e = format!(
            "taint_file={} tid={} shmid={} pipeid=200",
            &self.cmd.out_file, &id, &self.shmid
        );
        debug!("Track {}, e is {}", &id, e);
        self.envs.insert(defs::TAINT_OPTIONS.to_string(), e);
    ...
    fn run_target(
        ...
        let ret = match child.wait_timeout(timeout) {
            ...
            Ok(None) => {
                ...
                child
                    .kill()
                    .map_err(|err| warn!("child kill {:?}", err))
                    .ok();
                child
                    .wait()
                    .map_err(|err| warn!("child wait {:?}", err))
                    .ok();
                StatusType::Timeout
            }
