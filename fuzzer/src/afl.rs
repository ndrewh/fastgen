...
use rand::{self, distributions::Uniform, thread_rng, Rng, RngCore};
...
pub fn mutate(
    ...
    if buf.len() == new_size {
        ...
        let mut i = 0;
        while i < new_size - sol.len() - field_index && i < buf.len() - field_size - field_index {
...
pub fn run_afl_mutator(executor: &mut Executor, buf: &mut Vec<u8>) {
    ...
    let max_stacking = if buf.len() <= 16 { 64 } else { 256 };
    /*
