...
pub fn map_args(
    ...
    for i in 0..node.get_children().len() {
        ...
        map_args(
            c, local_map, shape, input_args, inputs, visited, const_num, buf,
        );
    }
    ...
    match FromPrimitive::from_u32(node.get_kind()) {
        Some(RGD::Constant) => {
            ...
            let iv = node
                .get_value()
                .parse::<u64>()
                .expect("expect u64 number in value field");
        ...
        _ => {
            if node.get_children().len() == 2 {
                let hash = xxhash(
                    (node.get_kind() << 16) | node.get_bits(),
                    node.get_children()[0].get_hash(),
                    node.get_children()[1].get_hash(),
                );
                node.set_hash(hash);
            }
        }
