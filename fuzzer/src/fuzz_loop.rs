    ...
    if config::SAVING_WHOLE {
        ...
        while running.load(Ordering::Relaxed) {
            ...
            if let Some(buf) = buf {
                ...
                info!(
                    "grading input {:?} it is a new input {}, saved as input {}",
                    fpath, new_path.0, new_path.1
                );
        ...
        while running.load(Ordering::Relaxed) {
            ...
            if let Some(mut buf) = depot.get_input_buf(sol.fid as usize) {
                ...
                {
                    flipped += 1;
                    branch_fliplist.write().unwrap().insert((
                        sol.addr,
                        sol.ctx,
                        sol.order,
                        sol.direction,
              ...
              fid, addr, ctx, order, direction, bid, sctx, new_path.0, new_path.1);
                    ...
                    if addr != 0
                        && branch_gencount
                            .read()
                            .unwrap()
                            .contains_key(&(addr, ctx, order, direction))
                    {
                        count = *branch_gencount
                            .read()
                            .unwrap()
                            .get(&(addr, ctx, order, direction))
                            .unwrap();
                        count += 1;
                    ...
                    branch_gencount
                        .write()
                        .unwrap()
                        .insert((addr, ctx, order, direction), count);
