...
uint32_t hashCallName(std::string str, std::string modname) {
  std::ifstream in(str, std::ifstream::ate | std::ifstream::binary);
  uint32_t fsize = in.tellg();
  uint32_t hash = 5381 + fsize * 223;
  for (auto c : str)
...
uint32_t hashName(std::string str) {
  std::ifstream in(str, std::ifstream::ate | std::ifstream::binary);
  uint32_t fsize = in.tellg();
  uint32_t hash = 5381 + fsize * 223;
  for (auto c : str)
...
void Taint::addContextRecording(Function &F) {
  ...
  uint32_t rr = hashCallName(fnNameStripped, ModName) % 1048576;
  ...
  //FIXME: we can't apply this patch
...
bool Taint::runOnModule(Module &M) {
  ...
  for (int itr = 0; itr< FnsToInstrument.size();itr++) {
    Function *i = FnsToInstrument[itr];
    bool addTo = addContext[itr];
